...
- 내부 슬롯과 내부 메서드의 내부는 자바스크립트 엔진 내부를 뜻한다
- 내부 슬롯과 내부 메서드는 자바스크립트 엔진의 내부 로직이므로 원칙적으로는 직접적으로 접근하거나 호출할 수 없지만, 일부에 한해서 간접적으로 접근할 수 있는 수단을 제공한다
- 예를 들어, 모든 객체는 [[Prototype]]이라는 내부 슬롯을 갖는다. 직접적으로 접근할 수는 없지만 __proto__를 통해 간접적으로 접근할 수 있다(참조의 개념이다)
```js
const o = {};
...
- 데이터 프로퍼티의 프로퍼티 어트리뷰트는 자바스크립트 엔진이 관리하는 내부 상태 값(meta-property)인 내부 슬롯 [[Value]], [[Writable]], [[Enumerable]], [[Configurable]]이다. 
...
// firstName {value: 'Dongchan', writable: true, enumerable: true, configurable: true}
...
![객체 변경 방지](https://user-images.githubusercontent.com/67866773/93660224-79402f80-fa87-11ea-9c1c-7116e171af10.PNG)
...
// 프로퍼티 추가가 금지된다
person.age = 28; // 무시. strict mode 에서는 에러
...
### 5-2. 객체 밀봉
- Object.seal 메서드는 객체를 밀봉한다. 객체 밀봉(seal)이란 프로퍼티 추가 및 삭제와 어트리뷰트 재정의 금지를 의미한다
...
### 5-4. 불변 객체
- 객체 확장 금지, 객체 밀봉, 객체 동결 메서드들은 얕은 변경 방지(shallow only)로 직속 프로퍼티만 변경이 방지되고 중첩 객체까지는 영향을 주지 못한다

**Object.freeze 메서드 중첩 객체 동결 불가 예시**
```js
const person = {
  name: 'Kim',
  address: { city: 'Seoul'}
...
console.log(person); // {name: 'Kim', address: {city: 'Incheon'}}
```
- **객체의 중첩 객체까지 동결하여 변경 불가능한 불변 객체(immutable object)를 구현하려면 객체를 값으로 갖는 모든 프로퍼티에 대해 재귀적으로 Object.freeze 메서드를 호출해야 한다**
...
- 프로퍼티는 데이터 프로퍼티와 접근자 프로퍼티가 있다
- 데이터 프로퍼티는 [[Value]], [[Writable]], [[Enumerable]], [[Configurable]] 의 4개의 프로퍼티 어트리뷰트를 갖는다
- 접근자 프로퍼티는 [[Get]], [[Set]], [[Enumerable]], [[Configurable]] 의 4개의 프로퍼티 어트리뷰트를 갖는다
- **프로퍼티 키는 문자열임을 잊지 말자**
