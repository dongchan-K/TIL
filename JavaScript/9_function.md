...
- 매개 변수에 할당할 수 있다는 것은 값이라는 의미이고 때문에 인수가 함수일 경우 값이기 때문에 매개변수에 할당 가능하다
...
- 단독으로 사용되는 문맥에서는 함수 리터럴이 아닌 선언문으로 자바스크립트 엔진에 의해 해석된다
- () 호출연산자 앞에 피연산자는 함수 객체(함수를 가리키는 식별자)가 와야한다
- 함수 객체는 호출 가능하지만 객체는 호출할 수 없다
- 즉시 실행 함수는 함수 정의와 동시에 호출되는 함수이다
- 즉시 실행 함수는 정의 후 즉시 실행한다
- 매개 변수는 함수 몸체 내에서만 변수와 동일한 방식으로 참조할 수 있다
- 재귀 함수는 함수 이름을 식별자로 함수 몸체 내에서 자기 자신을 호출하는 함수이다
- 중첩 함수는 함수 내부에 또다른 함수가 정의된 것이다
- 중첩 함수는 상위 함수가 호출되어야 생성된다 -> 함수 호출이 안 될 경우도 있기 때문에 중첩 함수는 미리 생성되지 않는다
