...
class ClassComponent extends React.Component {
  ...
  // 1. defaultProps 정적 메서드로 props 초기(default)값 설정
  static defaultProps = {
    name: 'Dongchan',
    children: 'Hello'
  }
...
컴포넌트의 props 타입을 지정할 때는 propTypes를 사용한다.

propTypes를 사용하기 위해서는 import 구문을 사용하여 API를 불러와야 한다.
...
// name 값은 문자열 형태로 전달해야 함을 의미한다.
FunctionComponent.propTypes = {
  name: PropTypes.string
};
```

위 예시처럼 문자열을 전달해야 하지만 숫자를 전달했다. 즉, 타입이 일치하지 않는다면 값은 나타나지만 콘솔에 경고 메시지를 출력한다.

필수 propTypes를 설정하려면 `propTypes`를 지정할 때 뒤에 `.isRequired` 를 붙여 주면 된다.
...
defaultProps(초기값 설정)와 propTypes(타입 지정)는 필수는 아니지만 해당 컴포넌트에 어떤 props가 필요한지 쉽게 알 수 있어 개발 능률에 도움이 된다.
